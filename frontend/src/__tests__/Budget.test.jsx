...
test.describe("Budget Page", () => {
  ...
  test("should open and close Add Budget modal", async ({ page }) => {
    ...
    await expect(page.locator(".modal")).toBeVisible(); // replace with modal selector if custom
    await page.keyboard.press("Escape"); // or close button
  ...
  test("should display budget cards", async ({ page }) => {
    const budgetCards = page.locator("h3:text-is('Food')"); // or check .text-lg
  ...
  test("should delete a budget after confirmation", async ({ page }) => {
    ...
    if ((await deleteButtons.count()) > 0) {
      ...
      await expect(deleteButtons.first()).not.toBeVisible(); // Assuming it gets removed
    }
  });
